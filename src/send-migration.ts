...
import { MigrationDocumentationEmail } from '../emails/migration-documentation';

// Initialize Resend with API key from environment
const resend = new Resend(process.env.RESEND_API_KEY);

interface SendMigrationEmailOptions {
  to: string | string[];
  customerName?: string;
  companyName?: string;
  bannerUrl?: string;
...
export async function sendMigrationEmail({
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      subject: 'Your Migration Guide from dex',
      react: MigrationDocumentationEmail({
        customerName,
        companyName,
        bannerUrl,
        supportEmail,
...
// Customer list for migration documentation
// Total: 41 recipients
const customerList = [
